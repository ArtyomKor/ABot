        ...
        async def mute_callback(interaction):
            if interaction.user == ctx.author:
                if role in пользователь.roles:
                    ...
                    await пользователь.edit(mute=True, reason=f'ABot, с наилучшими пожеланиями от {ctx.author.display_name}', communication_disabled_until=datetime.timedelta(days = 0, hours = 0, minutes = minutes, seconds = 0))
            else:
                await ctx.respond('Вы не являетесь администратором!')
