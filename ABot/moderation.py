    ...
    async def callback(self, interaction: discord.Interaction):
        if self.interacted.id == interaction.user.id:
            ...
            await self.user.timeout(
                datetime.timedelta(seconds=float(seconds), days=float(days), minutes=float(minutes), hours=float(hours),
                                   weeks=float(weeks)), reason=f"{self.user} был замучен на {self.values[0]}")
